...
# Names of the HOPs that inductor lowers into an in-place-mutating ``while_loop``.
# Their carried buffers and the carry accumulators are mutated in place across iterations.
_HOP_LOWERED_TO_WHILE_LOOP_NAMES = (
    "scan",
    "map_impl",
    "while_loop",
...
def _is_hop_lowered_to_while_loop(target: object) -> bool:
    return any(
        target is getattr(torch.ops.higher_order, name, None)
        for name in _HOP_LOWERED_TO_WHILE_LOOP_NAMES
    )


def _graph_reaches_hop_lowered_to_while_loop(gm: torch.fx.GraphModule) -> bool:
    """Whether ``gm`` or any of its (recursive) subgraphs calls one of the HOPs
    that inductor lowers into an in-place-mutating while_loop."""
    for node in gm.graph.nodes:
        if node.op == "call_function" and _is_hop_lowered_to_while_loop(node.target):
            return True
    for submod in gm.children():
        if isinstance(submod, torch.fx.GraphModule) and (
            _graph_reaches_hop_lowered_to_while_loop(submod)
        ):
            return True
    return False


def find_saved_tensors_feeding_hops_lowered_to_while_loop(
    module: torch.fx.GraphModule,
) -> set[torch.fx.Node]:
    """
    Returns the set of ``module`` placeholders that transitively feed a higher
    order operator that inductor lowers into an in-place-mutating ``while_loop``.
    """
    if not _graph_reaches_hop_lowered_to_while_loop(module):
        return set()

    # A consumer is "risky" if it is one of the while_loop-lowered HOPs, or an
    # outer HOP whose subgraph args recursively reach one.
    def _reaches_via_subgraph(node: torch.fx.Node) -> bool:
        return isinstance(node.target, torch._ops.HigherOrderOperator) and any(
            arg.op == "get_attr"
            and isinstance(arg.target, str)
            and isinstance(
                (submod := getattr(node.graph.owning_module, arg.target, None)),
                torch.fx.GraphModule,
            )
            and _graph_reaches_hop_lowered_to_while_loop(submod)
            for arg in node._input_nodes
        )

    def _is_risky_consumer(node: torch.fx.Node) -> bool:
        return node.op == "call_function" and (
            _is_hop_lowered_to_while_loop(node.target) or _reaches_via_subgraph(node)
        )

    feeding: set[torch.fx.Node] = set()
    for ph in module.graph.find_nodes(op="placeholder"):
        seen: set[torch.fx.Node] = set()
        stack = [ph]
        while stack:
            node = stack.pop()
            for user in node.users:
                if _is_risky_consumer(user):
                    feeding.add(ph)
                    stack.clear()
                    break
                if user not in seen:
